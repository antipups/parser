...
def pre_parse():
    ...
    for each_podcast in util.check_new_podcast():  # проходимся по ВСЕМ подкастам
        if each_podcast.get('status_podcast') == 2:  # если подкаст нуждается в полной записи
...
def parse(each_podcast):
    ...
    while html.find('<item>') > -1:    # до тех пор пока находим новый выпуск
        ...
        if util.check_item(title_item, title_podcast, mp3):    # если такой выпуск уже есть, не выходим, а просто его пропускаем
